...
export async function degreeAndCertificate(t) {
    const degreeCertificateProg = Selector('div.header-degree-certificates-cont').child('p');
    ...
    const hrefSelector = Selector('#navbarText').child('ul').child('li:nth-child(1)').child('a').getAttribute('href');
    await t.expect(hrefSelector).eql(href)
